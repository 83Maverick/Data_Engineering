...
-- 9.0.0   Challenge Lab: Loading Structured Data
...
-- 9.2.0   Part I - Load Uncompressed, Pipe-delimited Data

-- 9.2.1   Set your context and make sure you have the standard lab objects.
...
CREATE OR REPLACE SCHEMA FALCON_db.PROMO_CATALOG_SALES;
USE SCHEMA FALCON_db.PROMO_CATALOG_SALES;


-- 9.2.2   Find the uncompressed files in the stage.
...
-- 9.2.3   Query the files in the stage to get an idea of what the data looks
--         like.
...
-- 9.2.4   Create a file format that will handle uncompressed, pipe-delimited
--         data, and name it sales_csv_uncomp_pipe_tbl.

-- 9.2.5   Query the files using your new file format.

-- 9.2.6   Create tables in your new schema (region, nation, customer, and
--         supplier).

-- 9.2.7   Load the data in the files into your tables.
--         Don’t forget to include your file format in the COPY INTO statement.

-- 9.2.8   Write queries to validate the load.

-- 9.3.0   Part II - Load Gzip Compressed, Comma-delimited Data

-- 9.3.1   Locate the three sets of files in the stage and path
...
-- 9.3.2   Query the files in the stage to get an idea of what the data looks
--         like.
...
-- 9.3.3   Create a file format that will handle Gzip compressed, comma-
--         delimited data, and name it sales_csv_gzip_comma_gz.

-- 9.3.4   Query the files using your new file format.

-- 9.3.5   Create your tables (orders, lineitem, and partsupp).

-- 9.3.6   Load the data in the files into your tables.
--         Don’t forget to include your file format in the COPY INTO statement.

-- 9.3.7   Write queries to validate the load.

-- 9.3.8   Run a query that joins all the tables and selects at least one column
...
-- 9.4.0   Part III - Transform Data During a Load

-- 9.4.1   Let’s examine the data as it sits in the file.
--         Let’s reuse that file format that has no delimiter from before. The
--         code could look like this.
...
-- 9.4.2   Create the part table with the new specification provided by your
--         project manager.
...
-- 9.4.3   Take a moment to familiarize yourself with the split_part() function.
...
-- 9.4.4   Let’s run a select of the top 100 rows to make sure we are
...
-- 9.5.0   Now We are Ready to Load the Data
--         See if you can build the COPY INTO statement on your own.

-- 9.5.1   Write queries to validate the load.

-- 9.5.2   If you need help, the solution is on the next page.
