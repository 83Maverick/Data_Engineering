...
-- 10.0.0  Unloading Structured Data
...
-- 10.1.0  Unload Table Data into a Table Stage in Pipe-delimited File Format
...
-- 10.1.2  Create a fresh version of the REGION table with five records to
--         unload.
...
-- 10.1.3  Create a file format called mypipeformat.
...
-- 10.1.4  Unload the data to the REGION table stage.
...
-- 10.1.5  List the stage and query the file to verify that the data is there.
...
-- 10.1.6  Remove the file from the region table’s stage and verify it is gone.
...
-- 10.2.0  Use a SQL statement Containing a JOIN to Unload Data into an Internal
--         Stage
...
-- 10.2.1  Do a SELECT with a JOIN on the region and nation tables.
...
-- 10.2.2  Create a named stage (you can give any name to a named stage).

CREATE OR REPLACE STAGE mystage;


-- 10.2.3  Unload the joined data into the stage you created.
...
-- 10.2.4  Verify the file is in the stage.
...
-- 10.2.5  Remove the file from the stage and verify removal.
...
-- 10.2.6  Remove the stage.

DROP STAGE mystage;


-- 10.2.7  Resize and suspend the virtual warehouse.
