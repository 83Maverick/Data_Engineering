...
-- 4.0.0   Working with Transactions
...
-- 4.1.0   Transaction Model and Concurrency Control
...
-- 4.2.0   Explore the Autocommit Default Value
...
-- 4.2.1   Set the context for SHEET-A.
...
-- 4.2.2   Create a table and insert some records.
...
-- 4.2.3   Show the session is set to AUTOCOMMIT by default.
...
-- 4.2.4   Run the query and confirm the data is available.
...
-- 4.2.5   Rollback the above INSERT statement.
...
-- 4.2.6   Re-run query to query from the table.

-- SHEET-A --
SELECT * FROM t1;
-- SHEET-A --

--         You should still see all three records. Since AUTOCOMMIT is true, the
--         three records that were inserted are already committed and there was
--         nothing to roll back.

-- 4.3.0   Change the Session Value for the Autocommit Parameter
--         Now, we’re going to set AUTOCOMMIT to FALSE and observe what happens
--         when we attempt to insert records into a table.

-- 4.3.1   Set the AUTOCOMMIT parameter to FALSE.
...
-- 4.3.2   Confirm that AUTOCOMMIT is set to FALSE.
...
-- 4.3.4   Run a query to check for the new records.
...
-- 4.3.5   SWITCH TO OTHER WORKSHEET WINDOW ——> SHEET-B
...
-- 4.3.7   Run the following query in Worksheet SHEET-B.
...
-- 4.3.8   SWITCH BACK TO OTHER WORKSHEET WINDOW ——> SHEET-A
--         Now, you’re going to roll back the uncommitted insert, then re-insert
--         and commit the transaction.

-- 4.3.9   Rollback the INSERT statement in the transaction originated in Lab
--         SHEET-A.
...
-- 4.3.10  Run the query to select from the table.
...
-- 4.3.11  Insert the two rows again in Lab SHEET-A worksheet.
...
-- 4.3.12  Execute a COMMIT statement and commit the two extra rows.
...
-- 4.3.14  SWITCH TO OTHER WORKSHEET WINDOW ——> SHEET-B

-- 4.3.15  Query the table again.
...
-- 4.3.16  SWITCH BACK TO OTHER WORKSHEET WINDOW ——> SHEET-A

-- 4.4.0   Explore Multi-Statement Transactions
--         Since AUTOCOMMIT is still set to FALSE, any INSERT statements will be
--         pending until committed. Multiple INSERT statements can be explicitly
--         committed or rolled back as a single transaction.
--         Here, you’ll practice executing several INSERT statements as a single
--         transaction.

-- 4.4.1   Start a multi-statement transaction by executing two insert
--         statements in SHEET-A.
...
-- 4.4.2   End the multi-statement transaction by running the COMMIT statement.
...
-- 4.4.3   Query the table to see the new rows added by the two INSERT
--         statements above.
...
-- 4.4.4   Insert a new row into the table.
...
-- 4.4.5   Execute a ROLLBACK statement.
...
-- 4.4.6   Confirm that AUTOCOMMIT in the current session is still set to FALSE:
...
--         By default, the LOCK_TIMEOUT parameter is set to 43200 seconds
--         (i.e. 12 hours).

-- 4.5.1   Run a SELECT statement on table t1.
...
-- 4.5.2   Issue the SHOW LOCKS command.
...
-- 4.5.3   Confirm that AUTOCOMMIT in the current session is still set to FALSE.
...
-- 4.5.5   Check that the record has been updated.
...
-- 4.5.6   Run the SHOW LOCKS command.
...
-- 4.5.7   SWITCH TO OTHER WORKSHEET WINDOW ——> SHEET-B

-- 4.5.8   Run the SHOW LOCKS command here, and you should see the same locking
...
-- 4.5.9   While remaining in SHEET-B, run the following DELETE statement to the
...
-- 4.5.10  SWITCH TO OTHER WORKSHEET WINDOW ——> SHEET-A

-- 4.5.11  Run the SHOW LOCKS command.
...
-- 4.5.12  Execute the ROLLBACK statement to end the transaction and release the
--         lock.
...
-- 4.5.13  Run the SHOW LOCKS command to show that the lock is released.

-- SHEET-A --
SHOW LOCKS;
-- SHEET-A --

--         The command in this step should return no record as the lock has been
--         released with the ROLLBACK statement.

-- 4.5.14  SWITCH TO OTHER WORKSHEET WINDOW ——> SHEET-B

-- 4.5.15  Check that the DELETE statement is completed now, as shown above,
...
-- 4.6.0   Release Locks and Terminate Transactions

-- 4.6.1   SWITCH BACK TO OTHER WORKSHEET WINDOW ——> SHEET-A

-- 4.6.2   Execute DELETE statement on table t1.
...
-- 4.6.3   SWITCH TO OTHER WORKSHEET WINDOW ——> SHEET-B

-- 4.6.4   Execute the SHOW LOCKS statement.
...
-- 4.6.5   Review the transaction id associated with the DELETE statement and
--         release the lock.
...
-- 4.6.6   Execute the SHOW LOCKS statement.

-- SHEET-B --
SHOW LOCKS;
-- SHEET-B --

--         The command should return no record as the lock has been released.

-- 4.6.7   SWITCH BACK TO OTHER WORKSHEET WINDOW ——> SHEET-A

-- 4.6.8   Execute the ROLLBACK statement.
--         Because the transaction was aborted from SHEET-B, all further
--         statements in SHEET-A are ignored until you issue a COMMIT or a
--         ROLLBACK.
...
-- 4.6.9   Suspend your virtual warehouse.
...
--         worksheet. In Snowsight, each worksheet creates its own Snowflake
--         session.
