...
-- 1.1.4   Review the Security Recommendation, but do not enable MFA now.
--         In the Security Recommendation dialog, click Not now.
--         In this Snowflake training environment, MFA is not required.
...
--         active panel.
--         Clicking the Snowflake logo in the upper left corner will bring you
...
-- 1.3.3   Optionally, upload a profile photo.

-- 1.3.4   Optionally, enter your email address.
--         If you enter an email address, a verification link will be sent to
--         the address you specify.

-- 1.3.5   Explore multi-factor authentication (MFA).
...
--         Close the MFA documentation tab.

-- 1.3.6   Save your profile changes.
--         Return to the Snowsight tab, and click Save.

-- 1.3.7   Optionally, set the Snowsight UI appearance to Dark mode.
--         Click the up arrow to the right side of the user menu, then select
--         Appearance > Dark. Switch back to the System mode if you don’t like
--         the Dark mode.

-- 1.3.8   Open the documentation from the user menu.
...
-- 1.3.9   Click the Reference link at the top of the page and then click the
--         SQL command reference link from the list returned.

-- 1.3.10  Browse the SQL command reference.
...
-- 1.3.11  Click the Snowflake logo in the upper left corner of the web page to
--         return to the main page.

-- 1.3.12  Search for virtual warehouse in the search bar.
...
-- 1.3.13  Return to the Snowsight UI.
...
--         To create a folder, click the blue pull-down (v) beside the blue plus
--         (+) button in the upper right corner of the screen. From the drop-
--         down list that displays, select Folder. In the New Folder dialog box,
--         name your folder Working with Snowsight and then click Create Folder.
--         You will now be positioned inside your empty folder.
...
-- 1.4.3   Return to the Worksheets page.
--         Click the Snowflake logo in the upper left corner, then in the left
--         navigation bar select Projects, then select Worksheets.

-- 1.4.4   Click the My Worksheets tab.
...
-- 1.4.7   Create a new worksheet.
--         Click the blue plus (+) button in the upper right corner. This will
--         open a new SQL worksheet inside your folder.
...
-- 1.4.8   Change the name of your worksheet to Empty Worksheet.
...
-- 1.4.9   Return to the Worksheets page.
--         Click the Snowflake logo in the upper left corner, then in the left
--         navigation bar select Projects, then select Worksheets.

-- 1.4.10  Click My Worksheets from the list of tabs under Worksheets.
--         Your new worksheet is listed, with the folder name to the right.

-- 1.4.11  Click Folders from the list of tabs and open your folder.
--         You will also see your new worksheet listed there.

-- 1.4.12  Open the worksheet containing your SQL file.
--         From the list of worksheets, click the one you first created by
--         loading a SQL file.

-- 1.4.13  Move the worksheet out of the folder.
...
CREATE OR REPLACE WAREHOUSE FALCON_wh WITH
...
--         see TRAINING_ROLE as your role and FALCON_wh as your virtual
--         warehouse.

-- 1.5.4   Ensure that the Databases tab is selected in the object browser.
--         In the object browser on the left, select the Databases tab.
--         Observe the list of databases that this role has access to.

-- 1.5.5   Change your role to PUBLIC.
...
-- 1.5.6   Change your role back to TRAINING_ROLE and reset your context.
--         To change your role with SQL, run this:
...
-- 1.5.7   Show the tables that are in your current context.
...
-- 1.6.1   Create a database called FALCON_db.

CREATE OR REPLACE DATABASE FALCON_db;


-- 1.6.2   Create a schema called my_schema in the database you just created.

CREATE OR REPLACE SCHEMA FALCON_db.my_schema;

USE SCHEMA FALCON_db.my_schema;
...
ALTER USER FALCON SET
   DEFAULT_ROLE = training_role
   DEFAULT_WAREHOUSE = FALCON_wh
   DEFAULT_NAMESPACE = FALCON_db.my_schema;
