...
-- 13.0.0  Snowflake Stored Procedures and Snowflake Scripting
...
-- 13.1.0  Part I - Move Data from One Table to Another Using a Stored Procedure
...
CREATE SCHEMA IF NOT EXISTS FALCON_DB.STORED_PROCS;
USE SCHEMA FALCON_DB.STORED_PROCS;


-- 13.1.2  Create stored procedures that will set up the data you need for this
--         exercise.
...
-- 13.1.3  Call the stored procedure to create the NATION table and verify it
--         contains 25 rows.
...
-- 13.1.4  Create a stored procedure to create the NATIONS_NEW table.
...
-- 13.1.5  Call the stored procedure to create the NATIONS_NEW table and verify
...
-- 13.1.6  Create a stored procedure that will insert the staged rows into the
--         NATION table.
...
-- 13.1.7  Create a stored procedure that calls the two previously created
--         stored procedures.
...
-- 13.1.8  Call the stored procedure to transfer the staged data from the
...
-- 13.1.9  Restore your data to the starting state.
...
-- 13.1.10 Use an IF-THEN statement.
...
-- 13.1.11 Restore your data to the starting state.
...
-- 13.1.12 Call the stored procedure but pass in Y instead of N.
...
-- 13.1.14 Run the statements below to reset the data.
...
-- 13.1.15 Call the stored procedure and verify the results.
...
-- 13.1.16 Add an option where the user can indicate no change to either table.
...
-- 13.1.17 Restore the data in the tables, call the stored procedure, and
--         evaluate the result.
...
-- 13.1.18 Create a log table for truncated rows.
...
-- 13.1.19 Create a stored procedure that will log all the rows to be truncated.
...
-- 13.1.20 Add the stored procedure to TRANSFER_STAGED_ROWS().
...
-- 13.1.21 Reset the table data and verify the data in the tables.
...
-- 13.1.22 Call the stored procedure and verify the rows in each table.
...
-- 13.2.0  Part II - Analyze SQL Stored Procedures vs. JavaScript Stored
--         Procedures
...
-- 13.2.2  Run the script below to create a SQL stored procedure.
...
-- 13.2.3  Look at the parameters and variables.
...
-- 13.2.4  Look at the BEGIN and END block.
...
-- 13.2.5  Look at the IF-THEN statement.
...
-- 13.2.6  Look at the EXECUTE IMMEDIATE statement.
...
-- 13.3.0  Try Out the Stored Procedure

-- 13.3.1  Change the current virtual warehouse to a SMALL.
--         Execute the statement below. Small is a valid size, so the stored
--         procedure will change the virtual warehouse size for you.

CALL CHANGEWHSIZESQL('FALCON_wh', 'small');


-- 13.3.2  Change the current virtual warehouse to an XLARGE, which is too large
--         for the script.
--         Execute the statement below. XLarge is too big, so the stored
--         procedure will let you know that and won’t change the virtual
--         warehouse size.

CALL CHANGEWHSIZESQL('FALCON_wh_wh', 'xlarge');


-- 13.3.3  Change the current virtual warehouse to a VERYSMALL, which is an
--         invalid size.
--         Execute the statement below. The size indicated is not a valid size,
--         so the stored procedure will let you know that.

CALL CHANGEWHSIZESQL('FALCON_wh', 'verysmall');


-- 13.4.0  Create a JavaScript Stored Procedure
...
-- 13.4.1  Run the statement below to create the JavaScript stored procedure.
...
-- 13.4.2  Call the stored procedure with a valid virtual warehouse size:

CALL changewhsize ('FALCON_wh', 'small');


-- 13.4.3  Call the stored procedure with a virtual warehouse that is too large:

CALL changewhsize ('FALCON_wh', 'XLARGE');


-- 13.4.4  Call the stored procedure with a non-existent virtual warehouse size:

CALL changewhsize ('FALCON_wh', 'verysmall');


-- 13.4.5  Resize and suspend the virtual warehouse.
