...
-- 7.0.0   Caching and Query Performance
...
-- 7.1.0   Access and Navigate the Query Profile
...
-- 7.1.2   Create some data to query.
...
--         our subsequent query doesn’t just pull from cached data.
...
-- 7.1.3   Execute the following simple query in your worksheet.
...
-- 7.1.4   Access the Query Profile.
--         Now, let’s view the Query Profile. To the right of the query results,
--         you will see the Query Details panel. Click the ellipsis shown in the
...
-- 7.1.5   Click the Query Details tab.
...
-- 7.1.6   Click the Query Profile tab.
...
-- 7.1.7   Click on Node A (TableScan[2]).
...
-- 7.1.8   Click on Node B (Aggregate[1]).
...
-- 7.1.9   Click on Node C (Result[0]).
...
-- 7.2.3   Run the following SQL statement.
...
-- 7.3.1   Run the SQL statement below.
...
-- 7.3.2   Look at Percentage scanned from cache under Statistics in the Query
--         Profile.
...
-- 7.3.4   Look at percentage scanned from cache under Statistics in the Query
--         Profile.
...
-- 7.3.5   Add columns, run the query, and check the Query Profile.
...
-- 7.4.1   Set the context and virtual warehouse size.

USE SCHEMA SNOWFLAKE_SAMPLE_DATA.TPCDS_SF10TCL;
ALTER WAREHOUSE FALCON_WH SET WAREHOUSE_SIZE = 'XSMALL';


-- 7.4.2   Execute a query with partition pruning.
...
-- 7.4.3   Execute a query without partition pruning and check the Query
--         Profile.
...
-- 7.5.0   Determine If Spillage Is Taking Place
...
-- 7.5.1   Clear the data cache.

ALTER WAREHOUSE FALCON_WH SUSPEND;


-- 7.5.2   Run a query that generates spillage.
...
-- 7.5.4   Check out the Query Profile.
...
-- 7.5.5   Run the modified nested query.
...
-- 7.6.0   Review the output from EXPLAIN
--         Now, let’s compare the output of EXPLAIN from both queries we ran to
--         see how they are different.

-- 7.6.1   Use EXPLAIN to see the plan for the first query.
...
-- 7.6.2   Sort the rows using the operation column.
--         Hover over the operation column header. Click the three vertical dots
--         that appear, and select the upward-pointing arrow to sort the column
--         in ascending order.
...
-- 7.6.3   Use EXPLAIN to see the plan for the second query.
...
--         there is one less aggregate row than in the previous plan (for a
...
-- 7.6.4   Change your virtual warehouse’s size, then suspend it.

ALTER WAREHOUSE FALCON_WH
...
--         and EXPLAIN will help you better understand how caching impacts your
--         query performance. This, in turn, will allow you to write better
