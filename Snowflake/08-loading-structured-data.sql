...
-- 8.1.2   Create a Region table. This table will be loaded from a source file.
...
-- 8.1.3   Use the list command below to list the files.
...
-- 8.1.4   Review the properties of the stage.
...
-- 8.1.5   Review the contents of region.tbl.
...
-- 8.1.6   Create a file format called MYPIPEFORMAT.
...
--         The COMPRESSION setting is optional; if unspecified, Snowflake will
--         auto-detect the compression format of the files to be loaded.
--         However, you can specify the compression type, if you want to load
--         files of a specific compression type. Note that we have explicitly
--         set COMPRESSION = NONE.
--         Next, we’ll confirm that the REGION table is empty.

-- 8.1.7   Execute the statement below.

SELECT * FROM REGION;


-- 8.1.8   Create and execute a COPY INTO statement.
...
-- 8.1.9   Select and review the data in the REGION table.

SELECT * FROM REGION;

--         If you have data, your load was successful!

-- 8.2.0   Load a Gzip Compressed File on an External Stage into a Table
...
-- 8.2.1   Empty the REGION Table in your schema.

TRUNCATE TABLE region;


-- 8.2.2   Confirm again that the region.tbl.gz file is in the external stage.

LIST @training_db.traininglab.ed_stage/load/lab_files/region.tbl;


-- 8.2.3   Create a file format called MYGZIPPIPEFORMAT.
...
-- 8.2.4   Reload the REGION table from the region.tbl.gz file.
...
-- 8.2.5   Query the table to confirm the data was successfully loaded.

SELECT * FROM region;


-- 8.2.6   Try to load an uncompressed file using a file format with Gzip
--         compression.
...
-- 8.2.7   Practice loading a file where compression format is auto-detected.
...
-- 8.3.1   Create the Nation table by running the statement below.
...
-- 8.3.2   Try to copy into the Nation table by executing the COPY INTO
--         statement below.
...
-- 8.3.3   Recreate the NATION table and execute the COPY INTO statement.
...
-- 8.3.4   Check only the first 10 rows.
...
-- 8.4.1   Recreate the table with all columns in the same order as they are in
--         the nation.tbl file.
...
-- 8.4.2   Write a query that generates errors.
...
-- 8.4.3   Attempt to load the data.
...
-- 8.4.4   Run the SELECT statement below to verify the contents of the table,
...
-- 8.4.5   Retry the insert with ON_ERROR = ABORT_STATEMENT.
...
-- 8.4.6   Retry the insert with ON_ERROR = SKIP_FILE_4.
...
-- 8.4.7   Run query to validate error.
--         Run the query below to validate that the table is empty.

SELECT * FROM nation;


-- 8.4.8   Retry the insert with ON_ERROR = SKIP_FILE_6.
--         With this statement, we’re telling Snowflake that we don’t want to
...
-- 8.5.1   Suspend your virtual warehouse.
...
-- 8.7.1   Create a FILE FORMAT to use in the COPY INTO command.
...
-- 8.7.2   Copy data into the table.
