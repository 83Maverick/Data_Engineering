...
-- 15.0.0  Creating, Starting, and Stopping a Task
...
-- 15.1.0  Part I - Create, Start, and Stop a Single Task
...
--         to do so could result in all students being locked out of their
--         30-day training accounts.

-- 15.1.1  Set your context.
...
CREATE OR REPLACE SCHEMA FALCON_DB.TASKS_SCHEMA;
USE SCHEMA FALCON_DB.TASKS_SCHEMA;


-- 15.1.2  Take a look at the ORDERS table.
...
-- 15.1.7  Execute the task immediately.
...
-- 15.1.8  View the task history to observe the state of the task.

SELECT 
   * 
FROM 
   TABLE(information_schema.task_history())
WHERE database_name = 'FALCON_DB';


-- 15.1.9  Observe progress over the next five minutes.
...
-- 15.1.10 Suspend and drop the task to make sure it does not continue to run.
...
--         that happens, all students will be locked out of their 30-day
--         training accounts.

-- 15.2.0  Part II - Create a Root Task with Child Tasks
...
-- 15.2.2  Create your new schemas.

CREATE OR REPLACE SCHEMA FALCON_db.airline_tasks;
CREATE OR REPLACE SCHEMA FALCON_db.airline_data;


-- 15.2.3  Create a table for staging new data.
--         Create this staging table in your airline_data schema.

CREATE OR REPLACE TRANSIENT TABLE FALCON_db.airline_data.staging (
    country VARCHAR,
    region VARCHAR,
    name VARCHAR,
    code1 VARCHAR(5),
...
-- 15.2.4  Create the tables that will hold the collected data.
--         These will also reside in the airline_data schema.

CREATE OR REPLACE TABLE FALCON_db.airline_data.us_airlines (
   country VARCHAR,
   region VARCHAR,
   name VARCHAR, 
   alliance VARCHAR);
           
CREATE OR REPLACE TABLE FALCON_db.airline_data.dive_destinations (
   country VARCHAR,
   name VARCHAR);


-- 15.2.5  Set your context to use the tasks schema.

USE SCHEMA FALCON_db.airline_tasks;


-- 15.2.6  Create a root task to copy data into the staging table.
--         You will use a serverless task because the load should take less than
--         one minute. This will save compute credits since a virtual warehouse
--         would be charged for a minimum of one minute plus any time needed to
--         auto-suspend.
...
AS
   COPY INTO 
      FALCON_db.airline_data.staging
   FROM 
      @training_db.traininglab.datasets_stage/sba_data/airline_info/airlines.csv;


-- 15.2.7  Create a child task to merge information on US airlines.
...
AS 
   MERGE INTO FALCON_db.airline_data.us_airlines u
   USING FALCON_db.airline_data.staging s
   ON u.name = s.name
...
-- 15.2.8  Create a child task to insert information about dive destinations.
...
CREATE TASK load_dive_destinations
   AFTER load_staging_table
AS
   INSERT INTO FALCON_db.airline_data.dive_destinations (country, name)
   SELECT country, name
   FROM FALCON_db.airline_data.staging
   WHERE country IN ('Bahamas', 'Cayman Islands', 'Turks and Caicos Islands');
  


-- 15.2.9  Create a task to truncate the staging table.
...
AS
   TRUNCATE TABLE FALCON_db.airline_data.staging;
...
-- 15.2.10 Verify that all your tasks have been created.

SHOW TASKS;


-- 15.2.11 Check the dependencies of the root task (load_staging_table):

SELECT 
   *
FROM 
   TABLE(information_schema.task_dependents
   (task_name => 'FALCON_db.airline_tasks.load_staging_table'));
...
-- 15.2.12 View only the direct children of the root task.

SELECT 
   *
FROM 
   TABLE(information_schema.task_dependents
   (task_name => 'FALCON_db.airline_tasks.load_staging_table', RECURSIVE => FALSE));
...
-- 15.2.13 Enable all the tasks.

SELECT SYSTEM$TASK_DEPENDENTS_ENABLE('FALCON_db.airline_tasks.load_staging_table');
...
-- 15.2.14 Execute the root task immediately.

EXECUTE TASK load_staging_table;


-- 15.2.15 Verify that your task has started.

SELECT 
   * 
FROM 
   TABLE(information_schema.task_history())
WHERE database_name = 'FALCON_DB' AND schema_name = 'AIRLINE_TASKS';


-- 15.2.16 Wait for the next run of the root task to start on its schedule.
...
-- 15.2.17 Check your tables to see if information has been added.
...
SELECT * FROM FALCON_db.airline_data.staging;

SELECT * FROM FALCON_db.airline_data.us_airlines
ORDER BY name;

SELECT * FROM FALCON_db.airline_data.dive_destinations
ORDER BY name;
...
-- 15.2.19 Truncate the dive_destinations table.

TRUNCATE TABLE FALCON_db.airline_data.dive_destinations;


-- 15.2.20 Replace the load_dive_destinations task with one that uses MERGE.

CREATE OR REPLACE TASK load_dive_destinations
   AFTER load_staging_table
AS
   MERGE INTO FALCON_db.airline_data.dive_destinations d
   USING FALCON_db.airline_data.staging s
   ON d.name = s.name
   WHEN NOT MATCHED 
...
-- 15.2.21 Resume the load_dive_destinations task.

ALTER TASK load_dive_destinations RESUME;


-- 15.2.22 Resume the root task.

ALTER TASK load_staging_table RESUME;


-- 15.2.23 Execute the root task immediately.
...
-- 15.2.24 Check the status of your task.
...
-- 15.2.25 Select from the destination tables.
...
-- 15.2.26 Drop the us_airlines table.
--         This will introduce an error into your task run.

DROP TABLE FALCON_db.airline_data.us_airlines;


-- 15.2.27 Wait until the load_us_airlines task fails.
...
FROM 
   TABLE(information_schema.task_history())
WHERE 
   database_name = 'FALCON_DB' AND state = 'FAILED';


-- 15.2.28 Look at the status of all tasks.
...
-- 15.2.31 Drop the schemas you created.

DROP SCHEMA FALCON_db.airline_tasks;
DROP SCHEMA FALCON_db.airline_data;

--         If the root task was still active, you could not drop the schema
--         containing the task.

-- 15.2.32 Suspend your virtual warehouse.
