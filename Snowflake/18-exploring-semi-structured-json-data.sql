...
-- 18.0.0  Exploring Semi-Structured JSON Data
--         In this lab, you will create a simple table with a VARIANT column.
--         You will work with that simple table before you go on to explore more
--         complex data using both dot notation syntax and the FLATTEN function.
...
--         about this data type and teaching the other analysts what you have
--         learned. You decide to begin by creating and then working with some
--         simple JSON data.
...
-- 18.1.0  Query Simple JSON Data Using Dot Notation Syntax

-- 18.1.1  Make sure you have the required objects and set your context for this
--         lab.
...
-- 18.1.2  Create a table with JSON data.
...
--         parse_json function to create the VARIANT data from JSON strings. It
--         also uses the $1 notation to refer to the first column in the table
--         and $2 to refer to the second column.
...
-- 18.1.3  Open a row of the table you just created to see its structure.
...
-- 18.1.4  Extract the name data from your table using dot notation syntax.
--         One way to extract values from VARIANT data is with dot notation
--         syntax.
--         Dot Notation Syntax:
--         In your SELECT statement, you would simply reference the column, add
--         a colon, and then reference the elements one by one, separated by
--         dots.
...
-- 18.1.5  Determine the data types in your result set.
...
-- 18.1.6  Rerun the query and examine the output.
...
--         types.
--         Let’s observe how we can convert VARIANT data to a standard SQL data
--         type.

-- 18.1.7  Extract the name data and cast it to type VARCHAR.
...
--         values. This allows you to sort them, apply string functions, or do
--         anything you can with structured data.

-- 18.1.8  Create a table from your query.
...
--         who do not know how to query semi-structured data directly.

-- 18.2.0  Query Nested JSON Data with Dot Notation
...
-- 18.2.1  Create a new version of your customers table with various levels of
--         nested JSON data.
...
--         brackets, you must do something extra with your dot notation to fetch
--         the values you want.

-- 18.2.2  Select from your table and click a cell in the INFO column to view
--         the structure.
...
-- 18.2.3  Query the table to fetch business and personal phone numbers.
...
--         the business phone number. That’s because we haven’t provided enough
--         detail in the query to specify which element in the array we want,
--         the first or the second.
--         Thus, in order to access the value of business (the first element in
--         the array), we have to specify an integer indicating a position in
--         the array (an index). In our example, the positions are zero for
--         business and one for personal. Remember, the first position is zero
...
--         and contact[1] for the personal phone number. These array details
--         specified the paths to the business and personal phone numbers. Then,
--         the dot notation (business.phone and personal.phone) specified the
--         values we wanted.

-- 18.2.4  Fetch all data and put it in structured format.
...
--         structured format so it can be used by colleagues who need to join it
--         with other structured data or don’t know how to work with it.

-- 18.3.0  Use FLATTEN to Query JSON Data
--         Now, we’re going to use the FLATTEN function (instead of dot
--         notation) to produce a structured representation of the same semi-
--         structured data we were working with earlier.
...
-- 18.3.1  Use the FLATTEN function to view your table’s top-level keys.
--         The top-level keys are the first level of key-value pairs processed
--         when parsing the semi-structured data indicated in your query.
...
--         When we use the FLATTEN function, its output is, in essence, joined
--         to table customers, and its columns can be selected just as you would
--         with columns from a table of structured data.
--         First, notice that the input of the FLATTEN function is
--         (input=\>\<column\>). For `<column>’, you can use just the column
--         name or a dot notation path like the ones we saw earlier in this lab.
--         Now, notice that your output contains eight columns. The first two
--         (ID and INFO) are columns from the customers table, and the last six
--         are the output of the FLATTEN function. Look at the key column. This
...
--         are the same keys from Jane Doe’s record. But what if you want to see
--         all the keys, even those nested in other structures?

-- 18.3.2  Use the RECURSIVE option to view all the keys from your table.
...
--         also flattened.
--         Note that when the index column shows zero (0), the value column
--         shows the business details. When the index column shows one (1), the
--         value column shows the personal details. This is consistent with what
--         we saw in the earlier dot notation exercise.

-- 18.3.3  Show only the keys that have simple values.
--         When you query semi-structured data, you typically access keys with
--         simple values. For example, you’re looking for the business phone
--         rather than the entire array of contact information. You can do this
--         with a WHERE clause that uses the function TYPEOF to check the type
...
--         array, but you filtered out all of the arrays with your WHERE clause.
--         So, there’s no point in including the index column in the query.
...
--         rows have a key of phone, but is that a business phone or a personal
--         phone? And is it for John or for Jane? The columns this, seq, and
--         path from the FLATTEN function can help answer these questions.

-- 18.3.4  Include the column this in your query.
...
--         If you look at the THIS column next to each phone number, you get a
--         good idea of who the phone number is for based on their email
--         address. However, email addresses do not always include someone’s
--         name, so that is not the best indicator.

-- 18.3.5  Include the path column in your query.
...
--         The path column is essentially the dot notation syntax representing
--         how you got to the value displayed. For a key of phone, you can see
--         that the path tells you whether it is a personal phone or a business
--         phone. But that still doesn’t solve the problem of who that phone
--         number belongs to.

-- 18.3.6  Include the seq column in your query.
...
--         multiple output rows, you know that all rows with the same sequence
--         value came out of the same row of data. Now, it’s easy to match up
--         phone numbers with the owner; you just look for the first and last
--         name with the same sequence number as the phone, and the path will
--         tell you whether it is a personal or a business phone.
--         In your output, it is very likely that the first six rows have a
--         sequence value of one, and the last six rows have a sequence value of
--         two. However, sequence numbers are not guaranteed to be displayed in
--         order. So, you should order your output by sequence number to ensure
--         that all the values from a single row are displayed together.
...
-- 18.3.7  Explode the data.
--         We want to list ID, first name, last name, business phone number and
--         email, and personal phone number and email. We need to explode the
--         data to do that. But first, we need to figure out the input into our
--         FLATTEN function.
...
--         Using what we learned earlier, we exploded all the data but filtered
--         to find only the rows where the index is not null. This gives us all
--         of the positions in the contact array.
...
-- 18.3.8  Fetch ID, first name, last name, and business details.
...
--         Fetching the ID and the first and last names was simple. For the
--         business details, we’ve inputted contact[0] into the FLATTEN function
--         to access only the business details.

-- 18.3.9  Fetch ID, first name, last name, and personal details.
--         Run the statement below:
...
-- 18.3.10 Fetch all contact details.
...
--         own. First, you’ll run a statement to create a small set of JSON
--         data; then, we’ll walk you through some challenge exercises.

-- 18.4.1  Setting the context.
...
CREATE OR REPLACE SCHEMA FALCON_DB.WEATHER;
USE SCHEMA FALCON_DB.WEATHER;


-- 18.4.2  Run the statement below to create the data you’ll query.
...
-- 18.4.3  Take a look at the following diagram to better understand the data
--         you just created.
...
--         contains objects, each with six elements: a key-value pair called dt
--         and five key-value pairs named air, atmospheric, sky, visibility, and
--         wind. With the exception of dt (which is a simple key-value pair),
--         the value in each of the key-value pairs is an object that, in turn,
--         contains a set of simple key-value pairs.
--         Station (the second key) has a corresponding value that is an object.
...
-- 18.4.4  Select USAF, WBAN, country, elev, id, and name from station using dot
--         notation.

-- 18.4.5  Select average, max, and min air temperature using LATERAL FLATTEN.

-- 18.4.6  Select max and min atmospheric pressure using LATERAL FLATTEN.

-- 18.4.7  Use either dot notation or FLATTEN LATERAL to fetch the dew point,
--         atmospheric pressure, and wind speed rate for the 15th observation.

-- 18.4.8  Check the solution just after the Key Takeaways if you need help.
--         Otherwise congratulations! You’ve just completed this lab.

-- 18.5.0  Key Takeaways
--         - JSON data is stored in a column of data type VARIANT. This is also
--         true for all other forms of semi-structured data capable of being
--         stored in Snowflake.
--         - While you can use VARIANT data types in comparisons or aggregations
--         with other data types, your queries won’t be as performant as they
--         would be if you had cast the VARIANT data to standard SQL data types.
...
--         index, value, and this to determine how your semi-structured data is
--         structured.
...
-- Use either dot notation or FLATTEN LATERAL to fetch the dew point, atmospheric pressure, and wind speed rate for the 15th observation. The index starts at 0, so the 15th observation has an index value of 14.
-- Dot notation:

 SELECT 
        wd.w:data.observations[14].air."dew-point"::NUMBER(38,1) AS dew_point,
        wd.w:data.observations[14].atmospheric.pressure::NUMBER(38,1) AS pressure,
        wd.w:data.observations[14].wind."speed-rate"::NUMBER(38,1) AS speed_rate
...
FROM 
    weather_data wd, 
    LATERAL FLATTEN (input => w:data.observations[14].air."dew-point", recursive => true) datapoint1,
    LATERAL FLATTEN (input => w:data.observations[14].atmospheric.pressure, recursive => true) datapoint2,
    LATERAL FLATTEN (input => w:data.observations[14].wind."speed-rate", recursive => true) datapoint3;
